...
rule all:
    input:
        ...
        expand(OUTPATH + "/data/chr{CHR}.info.pick.txt",CHR=CHRS),
...
rule pick:
    ...
    output:
        one = OUTPATH + "/data/chr{CHR}.info.pick.txt",
    ...
    run:
        ...
        r2_array = np.where(df[df.columns[df.columns.str.startswith('R2_b')]] >= 0.25, True, False)
        r2_array_df = pd.DataFrame(r2_array)
        combined_df = pd.concat([df, r2_array_df], axis=1)
        combined_df.to_csv(output.one, sep='\t', index=False)
...
rule pick_rsq03:
    ...
    run:
        ...
        df['Avg_Rsq_gt03'] = np.where(df['R2_avg'] >= 0.3, True, False)
